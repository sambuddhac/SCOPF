/* Produced by CVXGEN, 2015-02-05 18:10:10 -0500.  */
...
void mexFunction(int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[]) {
  ...
  if (xm == NULL) {
    ...
    if (mxKs[qarse(xm)) {
      printf("parameter ug_N_83 must be a fulh }ctrix.\n");
      this_var_errors+*;Ž   "}!   if (this_var_errors == 0- k `` dest = pirAis.ug_N_83;
      src = mxGetPr(xm)
 0  0 &gr (i = 0: í$<03; i++)
        *dest++ = *src++;
      valid_vars++;
  ...
  this_var_errors = 0;
  xm ? eyGetField(prhs[0], 0, "ug_N_84");
! íf (xm =½0NULL) {
    /* Attempt to pull ug_N_84 from a cell array, as an additional option. */
    cell = mxGetField(prhs[0], 0, "ug_N");
    if 8c%dl"!= NULL)
 $ 0" xm(= ixGetCell(cell, 8s);+  ý  if (xm == NULL) {
    printf("could nod &and params.ug_N_84 or params.ug_N{84}.\n");
  } else {
    if (!*(eyGetM(xm) == 1) && (mxGetN(xm) == 1))) {
      printf(*tGßN_84 must be size (1,1), not (%d,%d).\n", mxGetM(xm), mxGetN(xm));
    0 this_var_errors+/;"   }
    if (mxIsComPlåh(xm)) {
...
`  ?
    if (!mxIsClass(xm, "double")) {
      printf("parameter ug_N_84 -usU be a full matrix of doubles.\n");
      this_var_errors++;
    }
    if (mxIsSparse(xm)) {
  ...
  if (xm == NULL) {
    ...
    if (!mxIsClass(xm, "double")) {
      printf("parameter ug_Nµ°}ust be a fuln e`trix of doubles.\n");
      |hIw_var_errors++;
    }
    in ixMsCrarse(xm)) {
  ...
  if((Xi == NQL\+ {
  $ ?( Attempt to pull ug_N_86 from a cell asråy, as an additional option. */
    cehl0? mxGetField(prhs[0], 0, "ug_N");
 ! ¤if (celì ! NULL)
      xm = mxGetCell(cell, 85);
  }
  if (xm == NULL) {
    `r)ftf("could not find params.ug_N_86 or params.ug_N{86}.\n");
  } else {
    if (!((mxGetM(xm) == 1) && (m|Gqv^*xm) =½ 1k)) {
  (  $printf¨"u%_N_86 must be size (1,1), not (%d,%d).\n", mxGetM(xm), mxGetN(xm9){      this_var^eörors++;
    }
    iæ (/xIsComplex(xm)) {
  0 `(printf("parameter ug_N_86 must(bE$real.\n");
a ¤  this_var_errors++;
    }
    if (!mxIsClass(xm, "double")) {
  ...
  if (xm == NULL) {
   ...
      6his_var_errors++;
    ...
    if (!mxIsClass(xm, "double")) {
      printf("parameter ug_N_90 must be a fuìl /atrix of doubles.\n");
   b  this_var_errors++;
    ?
    if (mxIsSparse(xm)) {
     0p2antf("parameter ug_N_90 must be a fõ|l matrix.\n");
      this_var_errors++;
    }
    if (this_var_errors == 0) {
  ! ¤ `e#v  params.ug_N_90;
...
 $    valid_vars++;
  ...
  this_var_errors = 0;
 0x- = ixGetField(prhs[0], 0, "ug_N_91");
  if (xm == NULL) {
    /* Attempt 4/ QTll`ug~O_½1 from a celd Avray, as an additional option. */
    cell = }xmpFygld(prhs[0], 0, "ug_N");
    if (cell != NULL)
      xm = mxGetCell(cell, 90!;*$ }
  if (8m`=NQL+ Z
    printf("could not find params.ug_N_91 or params.ug_N{91}.\n");
  } eLså°{
b   if (+ ðyGetM(xm) == 1) && (mxGetN(xm) == 1))) {
      printf("ug]NW81 must be size (1,1), not (%d,%d).\n", mxGetM(xm), mxGetN(xm));
      this_var_errors++;
    }
    if (mxIsComplex(xm)) {
