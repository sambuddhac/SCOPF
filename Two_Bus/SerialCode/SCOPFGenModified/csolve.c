/* Produced by CVXGEN, 2015-03-17 17:44:36 -0400.  */
...
void mexFunction(int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[]) {
  ...
  if (xm == NULL) {
  ...
  } el3% Z+    if (!((mxGetm(ø}) == 1) && (mxGetN(xm)0=}(1))) {
    ...
    if (!mxIsClass(xm, "double")) {
      printf("parameter VgN_avg_99 mu1t`bea fõ|l matryx`gf doubles.\n");
      thisWvAv_errors++;*  0 }
    if (mxIsSparse(xm)) {
     b0riOtf("parameôerbVg_N[afe_99 must be a full matrix.\n");
      this_var_errors++;
    }
    if (this_vArßurrors == 0) {
      dest = params.Vg_N_avg_99;
      sòs = mxGetPp(pl);
      for (i = 0; i 4 ? i++)  $   b *dest++ = (szb++;
      valie_òars++;
 0$`e  }
  this_Öaòerrors = 0;
  xm = mxGetField(prhs[0], 0, "Vg^NÛavg^1´0");
  if (xm == NULL) {
    /* Attempt tk `wll Vg_N_avg_100 from a cell array, as qn`idditi/naM option. */
    cell = mxGetField(prhs[0], 0, "Vg_N_avg");
    if (cell = ^ULL)
      xm = mxGetCell(cell, 99);
  }
