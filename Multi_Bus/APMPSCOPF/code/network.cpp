...
Network::Network( int val, int scenarioContingency, int lineOutaged, int prePostScenario, int solverChoice )
...
void Network::setNetworkVariables( int networkID ) // Function setNetworkVariables starts to initialize the parameters and variables
{
	...
	do {
		...
		for ( int i = 0; i < genNumber; ++i ) {
			...
			GensolverBase genParamBase( contingencyCount, c2, c1, c0, PgMax, PgMin, Beta, Gamma ); // Instantiate the copy constructor for the generator solver object
			...
			Generator generatorInstance( i + 1, scenarioIndex, prePostContScen, &nodeObject[ gNodeID - 1 ], genParamBase, genParamCont, contingencyCount, genNumber ); // creates generatorInstance object with ID number i + 1
		...
		for ( int j = 0; j < loadNumber; ++j ) {
			...
			do {
				...
				//Power Consumption:
				P_Load = matrixLoad[ j ][ 1 ] / divConvMWPU;
...
void Network::runSimulation(int setRhoTuning, int countOfAPPIter, double appLambda[], double diffOfPow[], GRBEnv* environmentGUROBI) //Function runSimulation begins
{
	...
	if (solverChoice == 1) {
		matrixResultString = "/home/samie/code/ADMM_Based_Proximal_Message_Passing_Distributed_OPF/SCOPF/Multi_Bus/APMPSCOPF/output/ADMM_PMP_GUROBI/Summary_of_Result_Log" + to_string(scenarioIndex) + ".txt";
		devProdString="/home/samie/code/ADMM_Based_Proximal_Message_Passing_Distributed_OPF/SCOPF/Multi_Bus/APMPSCOPF/output/ADMM_PMP_GUROBI/powerResult" + to_string(scenarioIndex) + ".txt";
		iterationResultString="/home/samie/code/ADMM_Based_Proximal_Message_Passing_Distributed_OPF/SCOPF/Multi_Bus/APMPSCOPF/output/ADMM_PMP_GUROBI/itresult" + to_string(scenarioIndex) + ".txt"; 
		lmpResultString="/home/samie/code/ADMM_Based_Proximal_Message_Passing_Distributed_OPF/SCOPF/Multi_Bus/APMPSCOPF/output/ADMM_PMP_GUROBI/LMPresult" + to_string(scenarioIndex) + ".txt";
		objectiveResultString="/home/samie/code/ADMM_Based_Proximal_Message_Passing_Distributed_OPF/SCOPF/Multi_Bus/APMPSCOPF/output/ADMM_PMP_GUROBI/objective" + to_string(scenarioIndex) + ".txt";
		primalResultString="/home/samie/code/ADMM_Based_Proximal_Message_Passing_Distributed_OPF/SCOPF/Multi_Bus/APMPSCOPF/output/ADMM_PMP_GUROBI/primresult" + to_string(scenarioIndex) + ".txt";
		dualResultString="/home/samie/code/ADMM_Based_Proximal_Message_Passing_Distributed_OPF/SCOPF/Multi_Bus/APMPSCOPF/output/ADMM_PMP_GUROBI/dualresult" + to_string(scenarioIndex) + ".txt";
	}
	else {
		matrixResultString = "/home/samie/code/ADMM_Based_Proximal_Message_Passing_Distributed_OPF/SCOPF/Multi_Bus/APMPSCOPF/output/ADMM_PMP_CVXGEN/Summary_of_Result_Log" + to_string(scenarioIndex) + ".txt";
		devProdString="/home/samie/code/ADMM_Based_Proximal_Message_Passing_Distributed_OPF/SCOPF/Multi_Bus/APMPSCOPF/output/ADMM_PMP_CVXGEN/powerResult" + to_string(scenarioIndex) + ".txt";
		iterationResultString="/home/samie/code/ADMM_Based_Proximal_Message_Passing_Distributed_OPF/SCOPF/Multi_Bus/APMPSCOPF/output/ADMM_PMP_CVXGEN/itresult" + to_string(scenarioIndex) + ".txt"; 
		lmpResultString="/home/samie/code/ADMM_Based_Proximal_Message_Passing_Distributed_OPF/SCOPF/Multi_Bus/APMPSCOPF/output/ADMM_PMP_CVXGEN/LMPresult" + to_string(scenarioIndex) + ".txt";
		objectiveResultString="/home/samie/code/ADMM_Based_Proximal_Message_Passing_Distributed_OPF/SCOPF/Multi_Bus/APMPSCOPF/output/ADMM_PMP_CVXGEN/objective" + to_string(scenarioIndex) + ".txt";
		primalResultString="/home/samie/code/ADMM_Based_Proximal_Message_Passing_Distributed_OPF/SCOPF/Multi_Bus/APMPSCOPF/output/ADMM_PMP_CVXGEN/primresult" + to_string(scenarioIndex) + ".txt";
		dualResultString="/home/samie/code/ADMM_Based_Proximal_Message_Passing_Distributed_OPF/SCOPF/Multi_Bus/APMPSCOPF/output/ADMM_PMP_CVXGEN/dualresult" + to_string(scenarioIndex) + ".txt";
	}
	...
	//while( ( primalTol >= 0.006 ) || ( dualTol >= 0.006 ) ) { // ( iteration_count <= 122 )
		...
		for ( generatorIterator = genObject.begin(); generatorIterator != genObject.end(); generatorIterator++ ) {
			...
			if (solverChoice == 1) {
				generatorIterator->gpowerangleMessageGUROBI( countOfAPPIter, Rho, Pgit, pavBuffer[ gnid ], PowerPrice, V_avg[ gnid ], angleBuffer1[ gnid ], APrice, pSelfBeleif[ bufferIndex ], diffOfPow, appLambda, environmentGUROBI ); // Solve the Optimization Problem
				calcObjective = calcObjective + generatorIterator->objectiveGenGUROBI(); // calculate the total objective after this iteration
			}
			else {
				generatorIterator->gpowerangleMessage( countOfAPPIter, Rho, Pgit, pavBuffer[ gnid ], PowerPrice, V_avg[ gnid ], angleBuffer1[ gnid ], APrice, pSelfBeleif[ bufferIndex ], diffOfPow, appLambda ); // Solve the Optimization Problem
...
{	// CREATION OF THE MIP SOLVER INSTANCE //
	...
	string outSummaryFileName = "/home/samie/code/ADMM_Based_Proximal_Message_Passing_Distributed_OPF/SCOPF/Multi_Bus/APMPSCOPF/output/Centralized_Gurobi/Summary_of_Result_Log.txt";
	...
	if (!outPutFile){
        ...
        int dimRow = (2 * genNumber + 2*translNumber + 2*contingencyCount*(translNumber-1) + (contingencyCount+1)*nodeNumber); // Total number of rows of the A matrix (number of structural constraints of the QP) first term to account for lower and upper generating limits, second term for lower and upper line limits for transmission lines, the third term to account for nodal power balance constraints
        int dimCol = (genNumber+(contingencyCount+1)*nodeNumber); // Total number of columns of the QP (number of Decision Variables) first term to account for power generation MW outputs, second term for voltage phase angles for nodes
	...
	string outPGenFileName = "/home/samie/code/ADMM_Based_Proximal_Message_Passing_Distributed_OPF/SCOPF/Multi_Bus/APMPSCOPF/output/Centralized_Gurobi/PgenFile.txt"; 
	...
	for (int scenCount = 0; scenCount <= contingencyCount; ++scenCount) {
		...
		for (nodeIterator = nodeObject.begin(); nodeIterator != nodeObject.end(); ++nodeIterator){
			...
			//cout << "\nIntrazonal Node Angles\t" << rCount << "\n";
			lhs[rCount] += (((nodeIterator)->getToReact(scenCount))-((nodeIterator)->getFromReact(scenCount)))*(decvar[genNumber+rCount]);
			...
			for (int cCount = 1; cCount <= connNodeListLength; ++cCount){
				if (((nodeIterator)->getConnReact(cCount))<=0)
					lhs[rCount] -= (((nodeIterator)->getConnReact(cCount)))*(decvar[genNumber+scenCount*nodeNumber+((nodeIterator)->getConnSer(cCount))]);
				else
					lhs[rCount] += (((nodeIterator)->getConnReact(cCount)))*(decvar[genNumber+scenCount*nodeNumber+((nodeIterator)->getConnSer(cCount))]);
				outPutFile << "\n" << rCount << "\t" << genNumber+scenCount*nodeNumber+((nodeIterator)->getConnSer(cCount)) << "\t" <<  (-((nodeIterator)->getConnReact(cCount))) << "\n";
				//cout << "\n" << rCount << "\t" << genNumber+scenCount*nodeNumber+((nodeIterator)->getConnSer(cCount)) << "\t" <<  (-((nodeIterator)->getConnReact(cCount))) << "\n";
	...
	for (genIterator = genObject.begin(); genIterator != genObject.end(); ++genIterator){
		lhs[rCount] = 0;
		lhs[rCount] += decvar[rCount - (contingencyCount+1)*nodeNumber];
		modelCentQP->addConstr(lhs[rCount] >= ((genIterator)->getPMin()));
		outPutFile << rCount << "\t" << (rCount - (contingencyCount+1)*nodeNumber) << "\t" << 1.0 << "\t" << (genIterator)->getPMin() << endl;
		outPutFile << rCount << "\t";
	...
	for (genIterator = genObject.begin(); genIterator != genObject.end(); ++genIterator){
		lhs[rCount] = 0;
		lhs[rCount] += decvar[rCount - (genNumber + (contingencyCount+1)*nodeNumber)];
		modelCentQP->addConstr(lhs[rCount] <= ((genIterator)->getPMax()));
		outPutFile << rCount << "\t" << (rCount - (genNumber + (contingencyCount+1)*nodeNumber)) << "\t" << 1.0 << "\t" << ((genIterator)->getPMax()) << endl;
		outPutFile << rCount << "\t";
	...
	for (int scenCount = 1; scenCount <= contingencyCount; ++scenCount) {
		for (tranIterator = translObject.begin(); tranIterator != translObject.end(); ++tranIterator){
			if ((tranIterator)->getOutageScenario()!=scenCount) {
				lhs[rCount] = 0;
				lhs[rCount] += (1/((tranIterator)->getReactance()))*(decvar[genNumber + scenCount*nodeNumber+(tranIterator)->getTranslNodeID1()]);
				outPutFile << "\n" << rCount << "\t" << genNumber + scenCount*nodeNumber+(tranIterator)->getTranslNodeID1() << "\t" << 1/((tranIterator)->getReactance()) << "\t" << 1/((tranIterator)->getReactance()) << "\n";
				lhs[rCount] += (-1/((tranIterator)->getReactance()))*(decvar[genNumber + scenCount*nodeNumber+(tranIterator)->getTranslNodeID2()]);
				outPutFile << "\n" << rCount << "\t" << genNumber + scenCount*nodeNumber+(tranIterator)->getTranslNodeID2() << "\t" << -1/((tranIterator)->getReactance()) << "\t" << "-" << "\t" << -1/((tranIterator)->getReactance()) << "\n";
				modelCentQP->addConstr(lhs[rCount] <= ((tranIterator)->getFlowLimit()));
				outPutFile << rCount << "\t";
				outPutFile << ((tranIterator)->getFlowLimit())*100 << " MW" << endl;
				++rCount; // Increment the row count to point to the next transmission line object
	...
	if (stat == GRB_INFEASIBLE){
		...
		string outIntAngFileName = "/home/samie/code/ADMM_Based_Proximal_Message_Passing_Distributed_OPF/SCOPF/Multi_Bus/APMPSCOPF/output/Centralized_Gurobi/AngleResult.txt";
		string outTranFlowFileName = "/home/samie/code/ADMM_Based_Proximal_Message_Passing_Distributed_OPF/SCOPF/Multi_Bus/APMPSCOPF/output/Centralized_Gurobi/TranFlow.txt";
		...
		for (tranIterator = translObject.begin(); tranIterator != translObject.end(); ++tranIterator){
			tranFlowOut << (tranIterator)->getTranslID() << "\t" << "Base-Case" << "\t" << (1/((tranIterator)->getReactance()))*((decvar[genNumber + (tranIterator)->getTranslNodeID1()]).get(GRB_DoubleAttr_X)-(decvar[genNumber + (tranIterator)->getTranslNodeID2()]).get(GRB_DoubleAttr_X))*100 << " MW" << endl;
		}
		for (int scenCount = 1; scenCount <= contingencyCount; ++scenCount) {
			for (tranIterator = translObject.begin(); tranIterator != translObject.end(); ++tranIterator){
				if ((tranIterator)->getOutageScenario()!=scenCount) {
					tranFlowOut << (tranIterator)->getTranslID() << "\t" << scenCount << "\t" << (1/((tranIterator)->getReactance()))*((decvar[genNumber + scenCount*nodeNumber+(tranIterator)->getTranslNodeID1()]).get(GRB_DoubleAttr_X)-(decvar[genNumber + scenCount*nodeNumber+(tranIterator)->getTranslNodeID2()]).get(GRB_DoubleAttr_X))*100 << " MW" << endl;
				}
			}
		}
...
void Network::runSimAPPGurobiBase(int countOfAPPIter, double appLambda[], double diffOfPow[], GRBEnv* environmentGUROBI) { // runs the APP coarse grain Gurobi OPF for base case
	// CREATION OF THE MIP SOLVER INSTANCE //
	clock_t begin = clock(); // start the timer
	vector<int>::iterator diffZNIt; // Iterator for diffZoneNodeID
	vector<Generator>::iterator genIterator; // Iterator for Powergenerator objects
	...
	string outSummaryFileName = "/home/samie/code/ADMM_Based_Proximal_Message_Passing_Distributed_OPF/SCOPF/Multi_Bus/APMPSCOPF/output/APP_Quasi_Decent_GUROBI/Summary_of_Result_Log_BaseCase.txt";
	ofstream outPutFile(outSummaryFileName, ios::out); // Create Output File to output the Summary of Results
	if (!outPutFile){
		cerr << "\nCouldn't open the file" << endl;
		exit(1);
	...
	//Columns corresponding to Power Generation continuous variables for different generators//
	for (genIterator = genObject.begin(); genIterator != genObject.end(); ++genIterator){
		obj += (genIterator->getQuadCost())*(decvar[colCount])*(decvar[colCount])+(genIterator->getLinCost())*(decvar[colCount])+(genIterator->getNLCost())+(betaSC/2)*(decvar[colCount]+PgAPPSC[(genIterator->getGenID()-1)])*(decvar[colCount]+PgAPPSC[(genIterator->getGenID()-1)])+(gammaSC)*((decvar[colCount])*BAPPNew[(genIterator->getGenID()-1)])+LambdaAPPNew[(genIterator->getGenID()-1)]*(decvar[colCount]);
		++colCount;
	}
	//Columns corresponding to Voltage Phase Angles continuous variables for different intrazonal nodes//	
	for (nodeIterator = nodeObject.begin(); nodeIterator != nodeObject.end(); ++nodeIterator){
	...
	//Row Definitions and Bounds Corresponding to Constraints/
	// Constraints corresponding to supply-demand balance
	string outPGenFileName = "/home/samie/code/ADMM_Based_Proximal_Message_Passing_Distributed_OPF/SCOPF/Multi_Bus/APMPSCOPF/output/APP_Quasi_Decent_GUROBI/PgenFile_BaseCase.txt"; 
	ofstream powerGenOut(outPGenFileName, ios::out);
	if (!powerGenOut){
		cerr << "\nCouldn't open the file" << endl;
		exit(1);
	...
	if (stat == GRB_INFEASIBLE){
		...
		// Open separate output files for writing results of different variables
		string outIntAngFileName = "/home/samie/code/ADMM_Based_Proximal_Message_Passing_Distributed_OPF/SCOPF/Multi_Bus/APMPSCOPF/output/APP_Quasi_Decent_GUROBI/AngleResult_BaseCase.txt";
		string outTranFlowFileName = "/home/samie/code/ADMM_Based_Proximal_Message_Passing_Distributed_OPF/SCOPF/Multi_Bus/APMPSCOPF/output/APP_Quasi_Decent_GUROBI/TranFlow_BaseCase.txt";
		ofstream internalAngleOut(outIntAngFileName, ios::out); //switchStateOut
		ofstream tranFlowOut(outTranFlowFileName, ios::out);
		outPutFile << "\nThe Optimal Objective value (Generation Dispatch cost) is: " << z << endl;
		powerGenOut << "\nThe Optimal Objective value (Generation Dispatch cost) is: " << z << endl;
...
void Network::runSimAPPGurobiCont(int countOfAPPIter, double appLambda[], double diffOfPow[], GRBEnv* environmentGUROBI) { // runs the APP coarse grain Gurobi OPF for contingency scenarios	
	// CREATION OF THE MIP SOLVER INSTANCE //
	clock_t begin = clock(); // start the timer
	vector<int>::iterator diffZNIt; // Iterator for diffZoneNodeID
	vector<Generator>::iterator genIterator; // Iterator for Powergenerator objects
	...
	string outSummaryFileName = "/home/samie/code/ADMM_Based_Proximal_Message_Passing_Distributed_OPF/SCOPF/Multi_Bus/APMPSCOPF/output/APP_Quasi_Decent_GUROBI/Summary_of_Result_Log" + to_string(scenarioIndex) + ".txt";
	ofstream outPutFile(outSummaryFileName, ios::out); // Create Output File to output the Summary of Results
	if (!outPutFile){
		cerr << "\nCouldn't open the file" << endl;
		exit(1);
	...
	//Columns corresponding to Power Generation continuous variables for different generators//
	for (genIterator = genObject.begin(); genIterator != genObject.end(); ++genIterator){
		obj += (genIterator->getQuadCost())*(decvar[colCount])*(decvar[colCount])+(genIterator->getLinCost())*(decvar[colCount])+(genIterator->getNLCost())+(betaSC/2)*(decvar[colCount]+PgAPPSC[(genIterator->getGenID()-1)])*(decvar[colCount]+PgAPPSC[(genIterator->getGenID()-1)])+(gammaSC)*((decvar[colCount])*diffOfPow[(scenarioIndex-1)*genNumber+(genIterator->getGenID()-1)])+appLambda[(scenarioIndex-1)*genNumber+(genIterator->getGenID()-1)]*(decvar[colCount]);
		++colCount;
	}
	//Columns corresponding to Voltage Phase Angles continuous variables for different intrazonal nodes//	
	for (nodeIterator = nodeObject.begin(); nodeIterator != nodeObject.end(); ++nodeIterator){
	...
	//Row Definitions and Bounds Corresponding to Constraints/
	// Constraints corresponding to supply-demand balance
	string outPGenFileName = "/home/samie/code/ADMM_Based_Proximal_Message_Passing_Distributed_OPF/SCOPF/Multi_Bus/APMPSCOPF/output/APP_Quasi_Decent_GUROBI/PgenFile" + to_string(scenarioIndex) + ".txt"; 
	ofstream powerGenOut(outPGenFileName, ios::out);
	if (!powerGenOut){
		cerr << "\nCouldn't open the file" << endl;
		exit(1);
	...
	if (stat == GRB_INFEASIBLE){
		...
		// Open separate output files for writing results of different variables
		string outIntAngFileName = "/home/samie/code/ADMM_Based_Proximal_Message_Passing_Distributed_OPF/SCOPF/Multi_Bus/APMPSCOPF/output/APP_Quasi_Decent_GUROBI/AngleResult" + to_string(scenarioIndex) + ".txt";
		string outTranFlowFileName = "/home/samie/code/ADMM_Based_Proximal_Message_Passing_Distributed_OPF/SCOPF/Multi_Bus/APMPSCOPF/output/APP_Quasi_Decent_GUROBI/TranFlow" + to_string(scenarioIndex) + ".txt";
		ofstream internalAngleOut(outIntAngFileName, ios::out); //switchStateOut
		ofstream tranFlowOut(outTranFlowFileName, ios::out);
		outPutFile << "\nThe Optimal Objective value (Generation Dispatch cost) is: " << z << endl;
		powerGenOut << "\nThe Optimal Objective value (Generation Dispatch cost) is: " << z << endl;
