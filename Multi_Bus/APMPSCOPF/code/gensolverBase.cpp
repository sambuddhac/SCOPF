//solver.cpp
/* Produced by CVXGEN, 2017-08-04 19:28:13 -0400.  */
...
#include <cmath>
using namespace std;
double GensolverBase::eval_gap(void) {
  int i;
  double gap;
  gap = 0;
  for (i = 0; i < 2; i++)
...
void GensolverBase::set_defaults(void) {
...
void GensolverBase::setup_pointers(void) {
  work.y = work.x + 2;
  work.s = work.x + 2;
  work.z = work.x + 4;
  vars.Pg = work.x + 0;
...
void GensolverBase::setup_indexing(void) {
  setup_pointers();
}
void GensolverBase::set_start(void) {
  int i;
  for (i = 0; i < 2; i++)
    work.x[i] = 0;
  for (i = 0; i < 0; i++)
...
double GensolverBase::eval_objv(void) {
  int i;
  double objv;
...
void GensolverBase::fillrhs_aff(void) {
  int i;
  double *r1, *r2, *r3, *r4;
  r1 = work.rhs;
  r2 = work.rhs + 2;
...
void GensolverBase::fillrhs_cc(void) {
  int i;
  double *r2;
...
void GensolverBase::refine(double *target, double *var) {
  int i, j;
...
double GensolverBase::calc_ineq_resid_squared(void) {
...
double GensolverBase::calc_eq_resid_squared(void) {
...
void GensolverBase::better_start(void) {
...
void GensolverBase::fillrhs_start(void) {
...
void GensolverBase::tic(void) {
  tic_timestart = clock();
}
float GensolverBase::toc(void) {
...
float GensolverBase::tocq(void) {
  clock_t tic_timestop;
  tic_timestop = clock();
  return (float)(tic_timestop - tic_timestart) / CLOCKS_PER_SEC;
}
void GensolverBase::printmatrix(char *name, double *A, int m, int n, int sparse) {
  int i, j;
  printf("%s = [...\n", name);
  for (i = 0; i < m; i++) {
    for (j = 0; j < n; j++)
...
double GensolverBase::unif(double lower, double upper) {
...
#define EPS 1.2e-7
#define RNMX (1.0-EPS)
float GensolverBase::ran1(long*idum, int reset) {
  int j;
  long k;
  static long iy=0;
  static long iv[NTAB];
...
/* Next function is from numerical recipes in C. */
float GensolverBase::randn_internal(long *idum, int reset) {
  static int iset=0;
  static float gset;
  float fac, rsq, v1, v2;
  if (reset) {
...
double GensolverBase::randn(void) {
  return randn_internal(&global_seed, 0);
}
void GensolverBase::reset_rand(void) {
  srand(15);
  global_seed = 1;
...
void GensolverBase::multbymA(double *lhs, double *rhs) {
}
void GensolverBase::multbymAT(double *lhs, double *rhs) {
  lhs[0] = 0;
  lhs[1] = 0;
}
void GensolverBase::multbymG(double *lhs, double *rhs) {
  lhs[0] = -rhs[0]*(-1);
  lhs[1] = -rhs[0]*(1);
}
void GensolverBase::multbymGT(double *lhs, double *rhs) {
  lhs[0] = -rhs[0]*(-1)-rhs[1]*(1);
  lhs[1] = 0;
}
void GensolverBase::multbyP(double *lhs, double *rhs) {
...
void GensolverBase::ldl_solve(double *target, double *var) {
  int i;
...
void GensolverBase::ldl_factor(void) {
  work.d[0] = work.KKT[0];
  if (work.d[0] < 0)
...
double GensolverBase::check_factorization(void) {
...
void GensolverBase::matrix_multiply(double *result, double *source) {
...
double GensolverBase::check_residual(double *target, double *multiplicand) {
...
void GensolverBase::fill_KKT(void) {
...
//Workspace work;
//Settings settings;
GensolverBase::GensolverBase( int numberOfCases, double cost2, double cost1, double cost0, double PgenMax, double PgenMin, double Beta, double Gamma ) {
...
GensolverBase::~GensolverBase() {
}
#define NUMTESTS 0
void GensolverBase::mainsolve(int APPItCount, double gsRho, double Pgenprev, double Pgenavg, double Powerprice, double Angpriceavg, double Angavg, double Angprice, double PgenAPP, double BAPP[], double LambAPP1[]) {
  int num_iters;
#if (NUMTESTS > 0)
  int i;
  double time;
...
void GensolverBase::load_default_data(int APPItCount, double gsRho, double Pgenprev, double Pgenavg, double Powerprice, double Angpriceavg, double Angavg, double Angprice, double PgenAPP, double BAPP[], double LambAPP1[]) {
 if (APPItCount==1) {
  params.rho[0] = gsRho;
  params.PgAPPSC[0] = 0.0;
  for (int i=0; i<500; ++i) {
  ...
  for (int i=0; i<params.caseNumber; ++i) {
   params.BSC[i] = BAPP[i];
   params.lambda_1SC[i] = LambAPP1[i];
  }
 }
...
double GensolverBase::getPSol(void) {
 Piterate = (vars.Pg);
 return *Piterate;
}

double GensolverBase::getObj(void) {
 return (params.c2[0])*(*Piterate)*(*Piterate)+(params.c1[0])*(*Piterate)+(params.c0[0]);
}

double *GensolverBase::getThetaPtr(void){
Thiterate = (vars.Thetag);
return Thiterate;
}

double GensolverBase::getPMax() // start of getPMax function
{
	return params.PgMax[0];
} // end of getMax

double GensolverBase::getPMin() // start of getPMin function
{
	return params.PgMin[0];
} // end of getPMin

double GensolverBase::getQuadCost() // start of getQuadCoeff function
{
	return params.c2[0]; 
} // end of getQuadCoeff

double GensolverBase::getLinCost() // start of getLinCoeff function
{
	return params.c1[0]; 
} // end of getLinCoeff

double GensolverBase::getNLCost() // start of getConstCoeff function
{
	return params.c0[0]; 
} // end of getConstCoeff

double GensolverBase::getBeta(){return params.betaSC[0];}
double GensolverBase::getGamma(){return params.gammaSC[0];}
