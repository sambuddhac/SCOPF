...
void mexFunction(int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[]) {
  ...
  if (xm == NULL) {
    ...
    if (!((mxGetM(xm) == 1) && (mxGetN(xm) == 1))) {
      printf("BSC must be size (1,1), not (%d,%d).\n", mxGetM(xm), mxGetN(xm));
      this_var_errors++;
    }
    if (mxIsComplex(xm)) {
  ...
  this_var_errors = 0;
  xm = mxGetField(prhs[0], 0, "PgAPPSC");
  if (xm == NULL) {
    printf("could not find params.PgAPPSC.\n");
  } else {
    if (!((mxGetM(xm) == 1) && (mxGetN(xm) == 1))) {
      printf("PgAPPSC must be size (1,1), not (%d,%d).\n", mxGetM(xm), mxGetN(xm));
      this_var_errors++;
    }
    if (mxIsComplex(xm)) {
      printf("parameter PgAPPSC must be real.\n");
      this_var_errors++;
    }
    if (!mxIsClass(xm, "double")) {
      printf("parameter PgAPPSC must be a full matrix of doubles.\n");
      this_var_errors++;
    }
    if (mxIsSparse(xm)) {
      printf("parameter PgAPPSC must be a full matrix.\n");
      this_var_errors++;
    }
    if (this_var_errors == 0) {
      dest = params.PgAPPSC;
  ...
  this_var_errors = 0;
  xm = mxGetField(prhs[0], 0, "betaSC");
  if (xm == NULL) {
    printf("could not find params.betaSC.\n");
  } else {
    if (!((mxGetM(xm) == 1) && (mxGetN(xm) == 1))) {
      printf("betaSC must be size (1,1), not (%d,%d).\n", mxGetM(xm), mxGetN(xm));
      this_var_errors++;
    }
    if (mxIsComplex(xm)) {
      printf("parameter betaSC must be real.\n");
      this_var_errors++;
    }
    if (!mxIsClass(xm, "double")) {
      printf("parameter betaSC must be a full matrix of doubles.\n");
      this_var_errors++;
    }
    if (mxIsSparse(xm)) {
      printf("parameter betaSC must be a full matrix.\n");
      this_var_errors++;
    }
    if (this_var_errors == 0) {
      dest = params.betaSC;
  ...
  this_var_errors = 0;
  xm = mxGetField(prhs[0], 0, "lambda_2SC");
  if (xm == NULL) {
    printf("could not find params.lambda_2SC.\n");
  } else {
    if (!((mxGetM(xm) == 1) && (mxGetN(xm) == 1))) {
      printf("lambda_2SC must be size (1,1), not (%d,%d).\n", mxGetM(xm), mxGetN(xm));
      this_var_errors++;
    }
    if (mxIsComplex(xm)) {
      printf("parameter lambda_2SC must be real.\n");
      this_var_errors++;
    }
    if (!mxIsClass(xm, "double")) {
      printf("parameter lambda_2SC must be a full matrix of doubles.\n");
      this_var_errors++;
    }
    if (mxIsSparse(xm)) {
      printf("parameter lambda_2SC must be a full matrix.\n");
      this_var_errors++;
    }
    if (this_var_errors == 0) {
      dest = params.lambda_2SC;
      src = mxGetPr(xm);
      for (i = 0; i < 1; i++)
        *dest++ = *src++;
      valid_vars++;
  ...
  if (prepare_for_c) {
    ...
    for (i = 0; i < 1; i++)
      printf("  params.betaSC[%d] = %.6g;\n", i, params.betaSC[i]);
    for (i = 0; i < 1; i++)
      printf("  params.PgAPPSC[%d] = %.6g;\n", i, params.PgAPPSC[i]);
    for (i = 0; i < 1; i++)
      printf("  params.gammaSC[%d] = %.6g;\n", i, params.gammaSC[i]);
    for (i = 0; i < 1; i++)
      printf("  params.BSC[%d] = %.6g;\n", i, params.BSC[i]);
    for (i = 0; i < 1; i++)
      printf("  params.lambda_2SC[%d] = %.6g;\n", i, params.lambda_2SC[i]);
    for (i = 0; i < 1; i++)
